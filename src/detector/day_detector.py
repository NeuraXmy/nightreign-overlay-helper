...
class DayDetector:
    def __init__(self):
        ...
        for lang in config.dayx_detect_langs.keys():
            day1_image = Image.open(
                get_data_path(f"day_template/{lang}_1.png")
            ).convert("RGB")
            day2_image = Image.open(
                get_data_path(f"day_template/{lang}_2.png")
            ).convert("RGB")
            day3_image = Image.open(
                get_data_path(f"day_template/{lang}_3.png")
            ).convert("RGB")
            day1_mask = get_image_mask(
                resize_by_height_keep_aspect_ratio(
                    day1_image, config.template_standard_height
                )
            )
            day2_mask = get_image_mask(
                resize_by_height_keep_aspect_ratio(
                    day2_image, config.template_standard_height
                )
            )
            day3_mask = get_image_mask(
                resize_by_height_keep_aspect_ratio(
                    day3_image, config.template_standard_height
                )
            )
    ...
    def match(
        self, sct: MSSBase, template: DayTempalte, day1_region: tuple[int]
    ) -> tuple[bool, float]:
        try:
            config = Config.get()
            t = time.time()
            ...
            def match_region(region: tuple[int], template_mask: np.ndarray) -> float:
                ...
                img = sc.crop(region)
                img = resize_by_height_keep_aspect_ratio(
                    img, config.template_standard_height
                )
    ...
    def detect(self, sct: MSSBase, params: DayDetectParam | None) -> DayDetectResult:
        ...
        score_day1, score_day2, score_day3 = self.match(
            sct, template, params.day1_region
        )
        ret.score_day1 = score_day1
        ret.score_day2 = score_day2
        ret.score_day3 = score_day3
        if score_day1 < config.dayx_score_threshold:
            ret.start_day1 = True
        if score_day2 < config.dayx_score_threshold:
            ret.start_day2 = True
        if score_day3 < config.dayx_score_threshold:
            ret.start_day3 = True
        return ret
