...
POI_ICONS = {
    ctype: open_pil_image(f"icons/construct/{ctype}.png")
    for ctype in POI_ICON_SCALE.keys()
}
STD_POI_SIZE = (45, 45)
...
class MapDetector:
    def __init__(self):
        ...
        for ctype in self.info.all_poi_construct_type:
            ...
            for ct in self.all_unique_poi_image_ctype:
                if ctype // 1000 == ct // 1000 and POI_SUBICON_MAP.get(
                    ctype
                ) == POI_SUBICON_MAP.get(ct):
                    same_image_ct = ct
                    break
    ...
    def _match_earth_shifting(self, img: np.ndarray) -> tuple[int, float]:
        t = time.time()
        img = cv2.resize(
            img, PREDICT_EARTH_SHIFTING_SIZE, interpolation=CV2_RESIZE_METHOD
        )
        ...
        for map_id, map_img in MAP_BGS.items():
            ...
            for scale in np.linspace(min_scale, max_scale, scale_num):
                size = (
                    int(PREDICT_EARTH_SHIFTING_SIZE[0] * scale),
                ...
                for dx in range(-offset, offset + 1, stride):
                    for dy in range(-offset, offset + 1, stride):
                        map_shifted = map_resized[
                            y + dy : y + h + dy, x + dx : x + w + dx
                        ]
                        diff = np.abs(
                            (img.astype(np.float32) - map_shifted.astype(np.float32))
                        )
                        diff[diff > 100] = 0
            ...
            if score < best_score:
                ...
                best_map_id = map_id
        info(
            f"Match earth shifting: best map {best_map_id} score {best_score:.4f}, time cost: {time.time() - t:.4f}s"
        )
    ...
    def _match_poi(
        self, map_img: np.ndarray, map_bg: np.ndarray, pos: Position
    ) -> tuple[int, float]:
        img = map_img[
            pos[1] - STD_POI_SIZE[1] // 2 : pos[1]
            - STD_POI_SIZE[1] // 2
            + STD_POI_SIZE[1],
            pos[0] - STD_POI_SIZE[0] // 2 : pos[0]
            - STD_POI_SIZE[1] // 2
            + STD_POI_SIZE[0],
        ]
        bg = map_bg[
        ...
        for ctype, poi_img in self.all_unique_poi_images.items():
            ...
            for dx in range(-MAX_OFFSET, MAX_OFFSET + 1, OFFSET_STRIDE):
                for dy in range(-MAX_OFFSET, MAX_OFFSET + 1, OFFSET_STRIDE):
                    ...
                    img2 = cv2.resize(
                        img2, DOWNSAMPLE_SIZE, interpolation=CV2_RESIZE_METHOD
                    )
                    score = np.mean(
                        (img.astype(np.float32) - img2.astype(np.float32)) ** 2
                    )
    ...
    def _match_map_pattern(
        self, img: np.ndarray, earth_shifing: int
    ) -> tuple[MapPattern, int]:
        assert earth_shifing is not None, (
            "earth_shifing should be provided when matching map pattern"
        )
        ...
        # 识别POI
        map_bg = cv2.resize(
            MAP_BGS[earth_shifting], STD_MAP_SIZE, interpolation=CV2_RESIZE_METHOD
        )
        ...
        for x, y in sorted(self.info.all_poi_pos):
            ...
            paste_cv2(
                poi_result_img,
                np.array(self.all_unique_poi_images[ctype])[..., :3],
                (x - STD_POI_SIZE[0] // 2, y - STD_POI_SIZE[1] // 2),
            )
            # info(f"pos {x},{y} match construct {ctype} with score {score:.4f}")

        # 保存结果用于调试
        cv2.imwrite(get_appdata_path("map.jpg"), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        cv2.imwrite(
            get_appdata_path("map_poi_result.jpg"),
            cv2.cvtColor(poi_result_img, cv2.COLOR_RGB2BGR),
        )
        ...
        for pattern in self.info.patterns:
            ...
            for pos, ctype in poi_result.items():
                expect_ctype = pattern.pos_constructions.get(
                    pos, EMPTY_CONSTRUCTION
                ).type
        ...
        info(
            f"Match map pattern: best pattern by score: #{best_pattern_by_score.id} score: {best_score}"
        )
        info(
            f"Match map pattern: best pattern by error: #{best_pattern_by_error.id} error: {best_error}"
        )
        info(
            f"Match map pattern: return pattern #{best_pattern.id}, time cost: {time.time() - t:.4f}s"
        )
        return best_pattern, best_score

    def _draw_overlay_image(
        self, pattern: MapPattern, draw_size: tuple[int, int]
    ) -> Image.Image:
        ...
        extra_name = (
            get_name(pattern.day1_extra_boss) if pattern.day1_extra_boss != -1 else None
        )
        ...
        extra_name = (
            get_name(pattern.day2_extra_boss) if pattern.day2_extra_boss != -1 else None
        )
        icons.append(((x, y), NIGHT_CIRCLE_ICON))
        texts.append(
            (
        ...
        for pos, construct in pattern.pos_constructions.items():
            ...
            if (
                ctype // 1000 in (45, 46)
                and ctype // 100 != 460
                and (ctype == 45510 or ctype // 1000 != 45)
                and ctype not in (46780,)
            ):
                name = get_name(ctype)
        ...
        treasure = open_with_draw_size(
            f"treasures/treasure_{treasure_id}.png", (800, 800)
        )
        icons.append((scale_size((375, 375)), treasure))

        # 癫火塔
        if pattern.event_value == 3080:
            frenzy = open_with_draw_size(
                f"frenzy/Frenzy_{pattern.evpat_flag}.png", (800, 800)
            )
        ...
        # 保存结果用于调试
        img.convert("RGB").save(get_appdata_path("map_overlay_result.jpg"))
